...
이 수업에서는 **Codex**를 코드 작업을 돕는 AI로 사용합니다. 프로젝트 폴더를 연 뒤 Codex에 아래처럼 요청하세요.
...
LIVE 컴파일 후, 웹 화면에서 **AI 답변**을 누르기 전에 아래 명령을 실행하세요.

```bash
uv run python scripts/claims.py build
```

현재 위키 전체의 출처를 확인하고 `claims.jsonl`에 답변 근거를 저장합니다.
이 명령 자체는 LLM을 호출하지 않습니다. 메모를 수정하고 위키를 다시 컴파일했을 때도
실행하세요. `claim ledger written`이 나오면 웹 화면에서 관련 메모를 검색하고
**AI 답변**을 누릅니다. 답변과 출처가 함께 표시되는지 확인하세요.
AI 답변은 API 키가 필요하고 질문마다 비용이 발생합니다.

근거가 없어서 답변을 보류하면 화면에 안내된 같은 명령을 실행한 뒤 다시 시도하세요.
출처 오류가 표시되면 해당 위키의 원문 경로를 확인하고 컴파일부터 다시 실행하세요.
특정 페이지만 재생성하는 `--slug 페이지이름`은 전체 원장을 그 페이지들로 교체하므로,
일반 실습에서는 위의 전체 생성 명령을 사용합니다.
