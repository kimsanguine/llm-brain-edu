...
서로 다른 직무의 사람들이 쌓은 메모 **14개**입니다. (실명·실제 정보 없음)
...
notes/       # 메모 4개 (도구 스택 · 마케팅 · 행정 · 이커머스 CS)
```

`notes/` 의 뒤쪽 세 개는 **여러분 같은 실무자의 메모**입니다.

| 파일 | 누구의 메모인가 | 무엇이 반복되나 |
|---|---|---|
| `2026-01-24-campaign-weekly.md` | 마케팅 담당 | 매주 같은 성과표를 다시 만든다 |
| `2026-01-25-research-fund-rules.md` | 연구비·총무 행정 | 같은 규정 질문에 매번 근거를 다시 찾는다 |
| `2026-01-26-cs-repeat-questions.md` | 이커머스 1인 운영 | 지난 CS 답변을 못 찾아 매번 새로 쓴다 |

이 셋은 **`반복업무` 라는 같은 태그**로 묶여 있습니다. 직무는 다른데 고민이 같습니다.
"지난번에 뭐라고 했더라"를 못 찾는 것. 브레인이 없애려는 게 바로 그 시간입니다.

> 자기 직무에 가까운 메모를 먼저 읽어 보세요. 8주 뒤 여러분 브레인에는
> 이 자리에 **여러분 메모**가 들어가 있어야 합니다.
